...
<header>
  <div class="header-bar">
    ...
    <nav class="shared-nav" aria-label="Main Navigation">
      ...
      <a href="https://studio.youtube.com/video/l7bc8KTOyMo/edit" class="nav-item" target="_blank" rel="noopener noreferrer" style="color:#ef4444;" title="YouTube Studio Video Draft (l7bc8KTOyMo)"><span>▶️</span> YouTube Draft ↗</a>
    </nav>
...
- Scene 6 [02:33 - 03:20]: Resolution: Second brain awakening + Free Sunday live cohort build CTA.</textarea>
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; border-top:1px solid var(--line); padding-top:14px;">
      ...
      <div style="display:flex; flex-wrap:wrap; gap:8px;">
        ...
        <button type="button" class="btn" id="btnRunFalGen" onclick="runFalAudioGeneration()" style="background:#f59e0b; border-color:#d97706; color:#000; font-weight:800;" title="Generate speech for all 200 seconds of the voiceover script using Fal.ai">
          ⚡ Generate All Audio (Fal.ai)
        </button>
...
async function runFalAudioGeneration(){{
  ...
  if(!allSpokenText){{
    alert("Please ensure the voiceover script contains text before generating all audio.");
    return;
  }}
  ...
  if(btnRun){{
    btnRun.disabled = true;
    btnRun.textContent = "⚡ Generating All Audio...";
  }}
  ...
  try {{
    const res = await fetch("/api/ai/tts", {{
      method: "POST",
      headers: {{ "Content-Type": "application/json" }},
      body: JSON.stringify({{ engine: "fal", text: allSpokenText }}),
    }});
    const data = await res.json();
    if(data.ok){{
      const audioSrc = data.audio_base64 || data.audio_url;
      ...
      if(outputEl) outputEl.value = `⚡ COMPLETE FAL.AI VOICEOVER GENERATED (ALL 200s / ${{wordCount}} WORDS)!\nEngine: ${{data.engine}}\nSource: ${{data.key_source}}\nAudio Stream: ${{audioSrc ? "Ready & Playing" : "Generated"}}\n\n---\nFull Spoken Voiceover Script:\n${{allSpokenText}}`;
      if(statusEl) statusEl.textContent = `✅ Fal.ai Generated ALL Audio (${{wordCount}} words)!`;

      // Save complete audio generation to Azure with versioning
      try {{
        const saveRes = await fetch("/api/state", {{
          method: "POST",
          headers: {{ "Content-Type": "application/json" }},
          body: JSON.stringify({{
            ...
            testType: "fal_full_200s_audio",
            audioEngine: "fal.ai",
            fullSpokenText: allSpokenText,
            wordCount: wordCount,
            savedAt: new Date().toISOString(),
            backup: true
          }})
    ...
    if(btnRun){{
      btnRun.disabled = false;
      btnRun.textContent = "⚡ Generate All Audio (Fal.ai)";
    }}
  }}
}}
